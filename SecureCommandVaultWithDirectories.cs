...
{
    /// <summary>
    /// Universal Secure Command Vault - OTP-protected access to encrypted executables and directories
    /// Commands and directories are encrypted at execution time and can only run through this program
    /// </summary>
    public class SecureCommandVault : IDisposable
    {
        ...
        public async Task<VaultSetupResult> SetupVaultAuthentication(string accountName, string issuer = "SecureCommandVault")
        {
            try
            {
                var setupResult = await _totpManager.SetupTotp(accountName, issuer);
                
                _logger?.LogInformation("Vault TOTP setup completed for: {Account}", accountName);
        ...
        /// <summary>
        /// Authenticate for vault access using TOTP code
        /// </summary>
        public async Task<AuthenticationResult> AuthenticateForAccess(string totpCode)
        {
            try
            {
                var isValid = await _totpManager.ValidateTotp(totpCode);
                ...
                if (isValid)
                {
                    ...
                    _logger?.LogInformation("Vault access granted - Session: {SessionId}", sessionId);
                    ...
                    return new AuthenticationResult
                    {
                        ...
                        Message = "Authentication successful. Vault access granted."
                    };
                }
                else
                {
                    _logger?.LogWarning("Vault access denied - Invalid TOTP code");
                    return new AuthenticationResult
                    {
                        Success = false,
            ...
            {
                _logger?.LogError(ex, "Error during vault authentication");
                return new AuthenticationResult
                {
                    Success = false,
        ...
        /// <summary>
        /// Register a command executable in the secure vault
        /// </summary>
        public async Task<CommandRegistrationResult> RegisterSecureCommand(
            string sessionId,
            ...
            string[]? allowedArguments = null)
        {
            try
            {
                ...
                if (_availableCommands.ContainsKey(commandName))
                {
                    ...
                    {
                        Success = false,
                        Message = $"Command '{commandName}' already exists in vault"
                    };
                }

                _logger?.LogInformation("Registering command: {Command} from {Path}", commandName, executablePath);
